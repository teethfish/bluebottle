  ...
  if(tj < dom->Gfx._je && tk < dom->Gfx._ke) {
    for(int i = dom->Gfx._is; i < dom->Gfx._ie; i++) {
      ...
      u[i + tj*dom->Gfx._s1b + tk*dom->Gfx._s2b] = (u_star[i + tj*dom->Gfx._s1b
   	...
   	//printf("calculate_d u i %d  tj %d tk %d)%d is %f\n", i, tj, tk, i + tj*dom->Gfx._s1b + tk*dom->Gfx._s2b, u[i + tj*dom->Gfx._s1b + tk*dom->Gfx._s2b]);
      }
  }
}
  ...
  real *v, dom_struct *dom, real ddy, int *flag_v, int *phase)
{
  ...
  if(tk < dom->Gfy._ke && ti < dom->Gfy._ie) {
   ...
   //printf("calculate_d v ti %d  j %d tk %d)%d is %f\n", ti, j, tk, ti + j*dom->Gfy._s1b + tk*dom->Gfy._s2b, v[ti + j*dom->Gfy._s1b + tk*dom->Gfy._s2b]);
   }
  }
}
  ...
  real *w, dom_struct *dom, real ddz, int *flag_w, int *phase)
{
  ...
  if(ti < dom->Gfz._ie && tj < dom->Gfz._je) {
    for(int k = dom->Gfz._ks; k < dom->Gfz._ke; k++) {
  	...
  	//printf("calculate_d w ti %d tj %d k %d is %f\n", ti, tj, k, ti + tj*dom->Gfz._s1b + k*dom->Gfz._s2b, w[ti + tj*dom->Gfz._s1b + k*dom->Gfz._s2b]);
     }
  }
}
  ...
  dom_struct *dom, real dt0, real dt, int *phase)
{
  ...
  for(int i = dom->Gfx._is; i < dom->Gfx._ie; i++) {
    ...
    if((tj > 0 && tj < blockDim.x-1) && (tk > 0 && tk < blockDim.y-1)
     ...
     //printf("forcing on node (j %d k %d)%d is %f\n", j, k, i + j*dom->Gfx._s1b + k*dom->Gfx._s2b, f[i + j*dom->Gfx._s1b + k*dom->Gfx._s2b]); 
      // multiply by dt
    ...
    if((k >= dom->Gfx._ks && k < dom->Gfx._ke)
     ...
     //printf("u_star on node i %d j %d k %d %d is %f\n", i, j, k, i + j*dom->Gfx._s1b + k*dom->Gfx._s2b, u_star[i + j*dom->Gfx._s1b + k*dom->Gfx._s2b]); 
    }
  }
}
#endif
  ...
  dom_struct *dom, real dt0, real dt, int *phase)
{
  ...
  for(int j = dom->Gfy._js; j < dom->Gfy._je; j++) {
	...
	//printf("forcing y on node %d is %f\n", i + j*dom->Gfy._s1b + k*dom->Gfy._s2b, f[i + j*dom->Gfy._s1b + k*dom->Gfy._s2b]);
      // multiply by dt
